...
This screencast will show the processes involved in this project the first step is creating a model for the bank marketing data set using automl so the automl run is complete and we can see the uh models that were tried out by rml the best model is boarding ensemble which has the highest accuracy so since this is the model with higher with the highest accuracy, we can deploy this the model has been deployed using azure container instance which we can see if we go to the endpoint section the model has been deployed using a azure container instance and it has an end point now to interact with this deployed model we can use an api swagger is a tool which helps developers understand the api in a better way here swagger is showing the output input and output that this api uses so some of the inputs here is duration campaign example value etc and the output is a list containing yes or no now let's interact with this model  endpoint endpoint dot pi is a python script that contains two sets of inputs and the when we run this these inputs will be fed to the model using this api so after running endpoint not pi we can see the result here the first one was evaluated as yes and the second one was evaluated no sometimes we may need to re-run some of the steps and for that we can publish a pipeline in this project we have published a pipeline for the automl step if we follow the instructions in the notebook our pipeline will be published which we can see in the endpoints section so in the pipeline endpoints we can see that that our pipeline has been published here is the overview of the pipeline or the bank marketing data set and this data set will be fed to our auto ml module and this pipeline can be run whenever required using an uh using a http get request 
